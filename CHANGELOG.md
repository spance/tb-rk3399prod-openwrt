...
- 为 RK3399 AXI PCIe Host 增加 PI7C9X2G304SV `12d8:b304` 稀疏交换拓扑的
  安全扫描：交换芯片内部总线只探测 Device 1/2，空下行链路不发起无目标的
  ECAM 访问；下行 Link Status 使用不递归获取 PCI config lock 的驱动原始访问，
  并在本板首次总线扫描前预留 1.1 秒 RRS 就绪窗口，避免空 BDF 或 Retry 响应
  被控制器升级为同步外部总线异常。
- 交换板已经在 RK3399 上通过空板和单 RTL8125 冷启动枚举：空下行端口安全
  跳过，单卡形成完整 `upstream -> downstream -> 10ec:8125` 拓扑，未再出现
  同步外部总线异常、AER、timeout 或驱动复位。
- 单 RTL8125 经交换板双向各执行 300 秒、4 流 TCP，两个方向均达到
  2.35 Gbit/s。发送方向 0 重传；接收方向出现 1556 次 `rx_missed`/TCP 重传，
  约占 6444 万接收包的 0.0024%，作为满速接收边界接受。双端点、重复冷/热
  启动和长期压力测试仍待完成。
