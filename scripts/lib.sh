...
KERNEL_ABI_CONFIG="$PROJECT_DIR/configs/kernel-abi.conf"
[ -f "$KERNEL_ABI_CONFIG" ] || {
	echo "ERROR: kernel ABI baseline not found: $KERNEL_ABI_CONFIG" >&2
	exit 1
}
# shellcheck source=../configs/kernel-abi.conf
. "$KERNEL_ABI_CONFIG"
