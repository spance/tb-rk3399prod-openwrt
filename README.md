...
- `make all`：构建 U-Boot、默认启用 PCIe 的 OpenWrt FIT 和可写入原厂 `boot_linux` 分区的 ext2 镜像。
...
刷写使用 Rockchip 官方 RKDevTool/rkdeveloptool，并沿用开发板官方 loader、parameter/分区表。本工程不提供自动刷写或整盘 `dd` 脚本。
