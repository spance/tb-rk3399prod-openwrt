...
| USB3 Type-C | 硬件高速已确认，自动重连待验收 | 正反插 CC/角色/VBUS 正常，`usb-role-switch` 可随插拔删除/重建 xHCI。完整重绑 FUSB302、DWC3 和 `tcphy0` 后 Lexar E300 以 UAS/5000M 枚举，4 GiB direct read 约 367 MB/s（350 MiB/s），无 USB/UAS/SCSI/I/O 错误。上一版已进入 PHY `set_mode` 且无 PMA/PIPE timeout，但 Linux 6.12 动态路径先创建 xHCI、后设置 PHY，正反方向均报 `connect-debounce failed`；当前代码将动态路径对齐静态 host 顺序：DWC3 选定 host 后先重建 PHY，再创建 xHCI，待新镜像完成自动枚举、交替方向热插拔、长时读写和 Loader 回归 |
