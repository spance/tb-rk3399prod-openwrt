...
| PI7C9X2G304SV PCIe 交换板 | RK3399 空板和单端点已确认 | 空板能枚举 `12d8:b304` 上游口及 Device 1/2 两个下游桥，并安全跳过两个 inactive 下行链路；安装 RTL8125 后形成 `00:00.0 -> 01:00.0 -> 02:01.0 -> 03:00.0 [10ec:8125]`，另一端口保持空闲。两种冷启动均继续进入系统且无同步外部总线异常；单卡双向各 300 秒均为 2.35 Gbit/s，无 AER、timeout 或驱动复位。双端点、重复重启和长期压力仍待闭环 |
...
- PI7C9X2G304SV 交换扩展板已经在 RK3399 上通过空板和单 RTL8125 的修复后枚举及双向线速验收，证明空 BDF 规避和一个活动下行端口有效。满速接收 300 秒出现 1556 次 `rx_missed`/TCP 重传（约 0.0024%），没有 PCIe 或驱动错误，当前作为性能边界接受；第二下行端点、重复冷/热启动和长期稳定性仍未验证。验收顺序和阻断条件见 [PCIe RTL8125BG 2.5GbE](PCIE-RTL8125.md)。
