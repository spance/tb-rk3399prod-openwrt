...
- Linux 6.12 的 RK3399 Type-C PHY 缺少 TCPM orientation-switch 支持；工程回移 Rockchip 2026 年 v15 方案的 USB 部分。每次新 attachment（包括同方向重插）都记录方向、停止旧 PHY 并保持 USB3 host GRF 路由开启；DWC3 完成 role core reset 并选定 host 后，通过 generic PHY `.set_mode(PHY_MODE_USB_HOST)` 按方向重建 PHY、验证 PIPE ready，再创建 xHCI。Linux 6.12 的静态 host 路径本来采用这一顺序，工程补丁将顺序相反的动态 role-switch 路径与之对齐。自动 role-switch 已确认能删除/重建 xHCI；完整重绑 FUSB302、DWC3 和 `tcphy0` 后，C 口用 Lexar E300 确认 UAS/`5000M`，4 GiB direct read 约 367 MB/s（350 MiB/s）且无新增 USB/UAS/SCSI/I/O 错误。上一版在 xHCI 创建后才重建 PHY，正反方向均以 `connect-debounce failed` 告终；新顺序仍需刷机回归，设计和测试方法见 [USB Type-C SuperSpeed 主机](USB-TYPE-C.md)。
