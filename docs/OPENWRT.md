...
- eMMC：HS400 Enhanced Strobe、ADMA；电气配置对齐 Toybrick 4.4 后恢复 Linux 6.12 原生 RK3399 CQHCI/CQE depth 16。`cmdq_en=1`、读写、重启和 overlay 持久化均已完成实机回归，未出现 CQE recovery、CRC/ADMA、MMC 或 EXT4 错误。
...
- PCIe/RTL8125BG：标准 x4 机械槽使用 Gen2 x4 host 配置；实卡已以 `5.0 GT/s x1`、2500/full 工作，Linux 主线 `r8169` 和 `rtl8125b-2.fw` 完成单向线速与双向并发短测。无端点时 training timeout 符合预期，详细证据和长稳边界见 [PCIe RTL8125BG 2.5GbE](PCIE-RTL8125.md)。
