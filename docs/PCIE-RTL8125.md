...
扩展板插槽同样不支持带电插拔。修复后的 RK3399 已通过空板和单 RTL8125
冷启动枚举：空板安全跳过两个 inactive 下行链路，单卡形成
`00:00.0 -> 01:00.0 -> 02:01.0 -> 03:00.0 [10ec:8125]`，且两种情况均未再
出现同步外部总线异常。单卡经交换板双向各执行 300 秒、4 流 TCP，两个方向
均为 2.35 Gbit/s；发送方向 0 重传，接收方向的 1556 次 TCP 重传与网卡
`rx_missed` 增量一致，约占 6444 万接收包的 0.0024%。测试期间没有 AER、
timeout、驱动复位或非人工链路抖动。

这证明安全扫描和一个活动下行端口已经达到单卡功能及短时性能交付条件；双端点、
重复冷/热启动和长期压力完成前，不宣称交换扩展板的全部扩展能力已经交付。
